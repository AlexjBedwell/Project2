...
void supress_common(void){para* p = para_first(strings1, count1);
para* q = para_first(strings2, count2);
int foundmatch = 0;
...
void init_options_files(int argc, const char* argv[]) {
  ...
  if (showversion) { version();  exit(0); }                                                   //shows version
...
//  showoptions(files[0], files[1]);
  ...
  if (report_identical && !different) { printf("The files are identical.\n\n");   exit(0); }      // reports identical files

  if (showbrief && different) { printf("The files are different.\n\n");   exit(0); }            //reports if different
//lleftcolumn
  if (showsidebyside&&!showleftcolumn&&!suppresscommon){left_column(); exit(0);}
//left column
  if (showsidebyside&&showleftcolumn){left_column(); exit(0);}
//supress common
  if (showsidebyside&&suppresscommon){supress_common(); exit(0);}
//unfied
  if(showsidebyside&&showunified){show_unfied(); exit(0);}
